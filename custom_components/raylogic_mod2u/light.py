"""Raylogic MOD2U dimmer platform.
...
from .const import DOMAIN, CH_TYPE_DIMMER, DEVICE_MODEL_NAME, DEVICE_MODEL_DESC
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    entities = [
        RaylogicMod2uLight(hass, entry, device, ch_num, state)
        for ch_num, state in device.channel_states.items()
        if state.get("type") == CH_TYPE_DIMMER
    ]
    if entities:
        _LOGGER.info("Setting up %d MOD2U dimmer channel(s) on %s", len(entities), device.ip)
        async_add_entities(entities)


class RaylogicMod2uLight(LightEntity):
    ...
    def __init__(self, hass, entry, device: RaylogicMod2uDevice, ch_num, initial_state):
        ...
        self._attr_name = f"mod2u_{suffix}_area{area}_ch{ch_num}_dimmer"
